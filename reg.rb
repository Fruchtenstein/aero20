...
#        puts "INSERT OR REPLACE INTO runners VALUES (#{session[:sid]},'#{fullname}', '#{session[:uname]}', '#{params[:email]}', 0, #{params[:volume]}, #{session[:sex]}, '#{session[:acctoken]}', '#{session[:reftoken]}', '#{session[:city]}', '#{session[:state]}', '#{session[:country]}')"
