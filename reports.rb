...
#puts r
puts "соревнования"
r = `litecli 2020.db -te "select runnername, date, round(distance,2) dist,strftime('%H:%M:%S',time,'unixepoch') duration,strftime('%M:%S',time/distance,'unixepoch') pace, 'https://strava.com/activities/'||runid from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid and workout_type=1"`
puts r
puts "самая длинная неделя"
r = `litecli 2020.db -te "select  runnername,sum(distance) d from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid group by log.runnerid order by d desc limit 5"`
puts r
r = `litecli 2020.db -te "select  runnername,sum(distance) d from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid and sex=0 group by log.runnerid order by d desc limit 5"`
puts r
puts "самая длинная пробежка"
r = `litecli 2020.db -te "select  runnername,date,round(distance,2) dist,strftime('%H:%M:%S',time,'unixepoch') duration,strftime('%M:%S',time/distance,'unixepoch') pace, 'https://strava.com/activities/'||runid from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid order by distance desc limit 5"` 
puts r
r = `litecli 2020.db -te "select  runnername,date,round(distance,2) dist,strftime('%H:%M:%S',time,'unixepoch') duration,strftime('%M:%S',time/distance,'unixepoch') pace, 'https://strava.com/activities/'||runid from log,runners where date>'#{bow}' and date<'#{eow}' and log.runnerid=runners.runnerid and sex=0 order by distance desc limit 5"` 
puts r
...
r = `litecli 2020.db -te "select  runnername, strftime('%M:%S',t/d,'unixepoch') pace, t/d, teamname FROM (SELECT runnerid, SUM(time) t, SUM(distance) d FROM log WHERE date>'#{bow}' AND date<'#{eow}' AND time>0 GROUP BY runnerid) l, runners, teams WHERE runners.runnerid=l.runnerid AND teams.teamid=runners.teamid ORDER BY pace LIMIT 5"`
puts r
